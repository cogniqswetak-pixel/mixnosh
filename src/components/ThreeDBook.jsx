...
import { ChevronLeft, ChevronRight, Bookmark, Sparkles, BookOpen, Utensils } from "lucide-react";
...
export default function ThreeDBook({ pages, activeLocation, onOpenBooking }) {
  ...
  return (
    <div className="w-full max-w-5xl mx-auto py-4 px-2 sm:px-4 select-none">
      ...
      {/* ── HARDCOVER 3D BOOK DISPLAY CONTAINER ── */}
      <div
        className="relative mx-auto rounded-3xl p-3 sm:p-6 md:p-8 bg-gradient-to-b from-[#2a1b14] via-[#1c120d] to-[#120b08] border-4 border-[#3d271d] shadow-[0_25px_60px_rgba(0,0,0,0.45)] transition-transform duration-500 hover:shadow-[0_30px_70px_rgba(249,115,22,0.15)]"
      ...
      >
        ...
        >
          {/* Stacked Paper Edge Effect (Left & Right margins simulate paper thickness) */}
          ...
          {/* Spine Center Crease & Deep Gradient Shadow */}
          ...
          <div className="relative w-full h-full min-h-[480px] sm:min-h-[520px] grid grid-cols-1 md:grid-cols-2">
            ...
            {/* ── LEFT PAGE SPREAD ── */}
            <div
              className={`p-6 sm:p-8 md:p-10 flex flex-col justify-between border-b md:border-b-0 md:border-r border-[#e3d8c5] relative transition-opacity duration-300 ${
                isFlipping && flipDirection === "prev" ? "opacity-40" : "opacity-100"
              }`}
              style={{
            ...
            >
              <div>
                ...
                <div className="space-y-6">
                  {activeSpread.leftPage.map((item, i) => (
            ...
            {/* ── RIGHT PAGE SPREAD ── */}
            <div
              className={`p-6 sm:p-8 md:p-10 flex flex-col justify-between relative transition-opacity duration-300 ${
                isFlipping && flipDirection === "next" ? "opacity-40" : "opacity-100"
              }`}
              style={{
            ...
            {/* ── ACTIVE ANIMATING 3D FLIPPING PAGE SHEET ── */}
            {isFlipping && (
              <div
                className={`absolute inset-0 z-40 pointer-events-none ${
                  flipDirection === "next" ? "animate-page-flip-next" : "animate-page-flip-prev"
                }`}
                style={{
              ...
              >
                {/* Dynamic Page Shadow Sweep Overlay */}
                <div className="absolute inset-0 bg-gradient-to-r from-black/25 via-white/20 to-black/20 animate-pulse pointer-events-none rounded-r-xl" />
              </div>
            )}
          </div>
        </div>

        {/* ── BOTTOM FLIP INSTRUCTION CONTROLS ── */}
